...
export default function AddSolarStationBlogForm(): JSX.Element {
  ...
  const handleChange = (
    ...
    // Case 1: Jodit editor (RichTextEditorField) passes (name, content)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      toast({
        ...
        description:
          "Please fill in all required fields (Title, Brief Description, Full Content, Author, Category).",
        variant: "destructive",
        duration: 5000,
      });
      return;
    ...
    const blogDataForApi: BlogPostApiPayload = {
      ...
      metaKeywords:
        blogData.metaKeywords || (blogData.tags ? blogData.tags.join(",") : ""), // Fallback to tags

      keywords:
        blogData.metaKeywords || (blogData.tags ? blogData.tags.join(",") : ""), // Map metaKeywords or tags to keywords
    ...
    if (imageFile) {
      toast({
        ...
        description:
          "An image file was selected. For permanent storage, a separate backend API for image uploads is needed. The blog will be submitted with the provided image URL or empty if not handled.",
    ...
    try {
      const response = await fetch(
        "https://api.solarstation.in/blogs/addBlog",
        {
          method: "POST",
          headers: {
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2 col-span-1 md:col-span-2">
            ...
            >
              <Info className="h-4 w-4 text-purple-500" /> Brief Description{" "}
              <span className="text-red-500">*</span>
            </Label>
            <Textarea
              id="briefDescription"
          ...
          <div className="space-y-2 col-span-1 md:col-span-2">
            <Label htmlFor="description" className="flex items-center gap-1">
              <FileText className="h-4 w-4 text-orange-500" /> Full Content{" "}
              <span className="text-red-500">*</span>
            </Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="author" className="flex items-center gap-1">
              <User className="h-4 w-4 text-gray-500" /> Author{" "}
              <span className="text-red-500">*</span>
            </Label>
            <Input
              id="author"
          ...
          <div className="space-y-2">
            <Label htmlFor="category" className="flex items-center gap-1">
              <Folder className="h-4 w-4 text-gray-500" /> Category{" "}
              <span className="text-red-500">*</span>
            </Label>
            <Input
              id="category"
            ...
            {imagePreviewUrl && (
              <div className="mt-4 p-2 border rounded-lg bg-gray-50 flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Local preview of selected image file.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2 col-span-1 md:col-span-2">
            <Label htmlFor="metaKeywords" className="flex items-center gap-1">
              {" "}
              {/* Corrected name */}
              <Hash className="h-4 w-4 text-pink-500" /> SEO Keywords
              (comma-separated)
            </Label>
            <Input
              id="metaKeywords"
          ...
          <div className="space-y-2 col-span-1 md:col-span-2">
            <Label htmlFor="blogTags" className="flex items-center gap-1">
              <Tag className="h-4 w-4 text-orange-500" /> Blog Tags
              (comma-separated)
            </Label>
            <Input
              id="blogTags"

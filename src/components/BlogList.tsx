...
import {
  SolarStationRawBlog,
  SolarStationApiResponse,
} from "../types/solarstation"; // SolarStation specific types
...
const fetchSolarStationBlogs = async (
  page: number,
  limit: number,
  term: string,
  transform: (raw: any) => BlogPost
): Promise<BlogPost[]> => {
  ...
  if (!apiResponse.data || !Array.isArray(apiResponse.data)) {
    throw new Error(
      "SolarStation API response does not contain a valid 'data' array."
    );
  }
  return apiResponse.data.map(transform);
};
const baseURL = import.meta.env.VITE_API_URL;
const fetchVidhemaBlogs = async (
  page: number,
  limit: number,
  term: string,
  accessToken: string,
...
export default function BlogList(): JSX.Element {
  ...
  const transformSolarStationBlog = (
    rawBlog: SolarStationRawBlog
  ): BlogPost => {
    return {
      id: rawBlog._id,
      title: rawBlog.title,
  ...
  const transformVidhemaBlog = (rawBlog: VidhemaRawBlog): BlogPost => {
    return {
      ...
      tags: Array.isArray(rawBlog.meta_titlemetatags)
        ? rawBlog.meta_titlemetatags
        : [],
  ...
  const fetchBlogs = async () => {
    ...
    try {
      ...
      if (websiteFilter === "vidhema.com") {
        ...
        if (searchTerm) {
          vidhemaFilterObj.where = {
            title: { like: `.*${searchTerm}.*`, options: "i" },
          };
        }

        const encodedFilter = encodeURIComponent(
          JSON.stringify(vidhemaFilterObj)
        );
        const url = `${baseURL}/blogs?filter=${encodedFilter}`;
        ...
        const rawVidhemaBlogs = Array.isArray(result) ? result : result.data; // Support both direct array or { data: [...] }
        console.log(
          `this is conosole rdt in vidm is ${currentPage}!`,
          rawVidhemaBlogs
        );

        if (!Array.isArray(rawVidhemaBlogs)) {
          throw new Error(
            "Vidhema API response is not a valid array of blogs."
          );
        }
      ...
      } else if (websiteFilter === "solarstation.in") {
        ...
        if (
          ...
          console.log(
            "SolarStation API Pagination Response (from object):",
            apiResponse.pagination
          ); // Log API's pagination
        } else {
          ...
          console.log(
            "SolarStation Client-Side Total Pages Calculation (fallback):",
            totalPagesFromApi
          ); // Log client-side calculation
        }
        // --- END OF ROBUST LOGIC FOR SOLARSTATION.IN ---

        console.log(
          "SolarStation API Raw Response Data Length:",
      ...
      } else if (websiteFilter === "convexai.io") {
        // NEW: Logic for ConvexAI blogs
        ...
        if (searchTerm) {
          allConvexaiBlogs = allConvexaiBlogs.filter(
            (blog) =>
        ...
        if (searchTerm) {
          combinedBlogs = combinedBlogs.filter(
            (blog) =>
              blog.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
              (blog.briefDescription &&
                blog.briefDescription
  ...
  const handleDelete = async (id: string): Promise<void> => {
    ...
    if (
      window.confirm(
        `Are you sure you want to delete "${blogToDelete.title}"? This blog is from ${blogToDelete.website}.`
      )
    ) {
      try {
        let response;
        if (blogToDelete.website === "solarstation.in") {
          ...
          response = await fetch(
            `${baseURL}/blogs/${id}?access_token=${vidhemaAccessToken}`,
            {
              method: "DELETE",
              headers: { "Content-Type": "application/json" },
            }
          );
        } else if (blogToDelete.website === "convexai.io") {
          // NEW: Handle ConvexAI deletion
          toast({
    ...
    <div className="space-y-6 p-6">
      ...
      {!loading && !error && blogs.length === 0 && (
        <div className="text-center p-8 bg-white rounded-xl shadow-lg text-gray-600">
          ...
          {!searchTerm && (
            <p className="text-gray-500">Add a new blog to get started!</p>
          )}
        </div>
      )}
            ...
            >
              <div className="flex items-start justify-between flex-wrap md:flex-nowrap gap-4">
                ...
                <div className="flex gap-2 ml-auto self-start flex-shrink-0">
                  ...
                  {(blog.website === "solarstation.in" ||
                    blog.website === "vidhema.com" ||
                    blog.website === "convexai.io") &&
                    blog.id && ( // ADDED: ConvexAI to deletion condition
                      <Button
                        size="icon"
                        variant="outline"
